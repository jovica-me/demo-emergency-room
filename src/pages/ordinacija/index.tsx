...
type SokSobe = RouterOutputs["ordinacije"]["getCurrentAll"];
...
const columnsAll = [
  ...
  columnsAllHelper.accessor("brojSobe", {
    header: () => "Broj Sobe",
    cell: (info) => info.getValue(),
  }),
  columnsAllHelper.accessor("imeOrdinacija", {
    header: () => "Ime Ordinacija",
    cell: (info) => info.getValue(),
  }),
];
...
function TablePoseteSvi() {
  ...
  const { isLoading, isError, data, error, refetch } =
    api.ordinacije.getCurrentAll.useQuery(page);
  ...
  return (
    <Table
      ...
      tableRowClick={(row, router) => {
        void router.push(`/ordinacija/${row.id}`);
      }}
    ></Table>
  );
}
