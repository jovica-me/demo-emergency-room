...
// create a reducer that will store and read jmbg
...
const JMBGContext = createContext<JMBGContextValue>({
  jmbg: 0n,
  setJmbg: () => {},
});
...
export default function Page({}) {
  ...
  const [jmbg, setJmbg] = useState<bigint>(0n);

  const didMount = useRef(false);

  useEffect(() => {
    if (didMount.current) setIsPosetaAddModalOpen(true);
  ...
  return (
    <MainLayout>
      ...
      <JMBGContext.Provider value={{ jmbg, setJmbg }}>
        <main className="container flex flex-col  p-4">
          ...
          />
          <AddPoseta
            isOpen={isPosetaAddModalOpen}
            setIsOpen={setIsPosetaAddModalOpen}
            JMBG={jmbg}
          />
          <TablePacijenti />
        </main>
...
function ActionButton({ jmbg }: { jmbg: bigint }) {
  const { setJmbg } = useContext(JMBGContext);
  return (
    <button
      onClick={() => {
        setJmbg(jmbg);
